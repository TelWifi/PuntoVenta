...
namespace AppAccounting
{
    class NumLetras
    {
        ...
        public String Numero_to_Letras(string codmon, decimal numero)
        {
            string cadentero = Convierte(((int)(numero)).ToString(), NumLetras.Tipo.Pronombre);

            string caddecimal = ((int)((decimal.Round(numero - (int)(numero), 2)) * 100)).ToString() + "/100";
            using (Franquisia1._1.appbosaEntities db = new Franquisia1._1.appbosaEntities())
            {
                var obj = db.maesgen.Where(a => a.idmaesgen.Equals("015") && a.clavemaesgen.Equals(codmon)).FirstOrDefault();
                if (obj!=null)
                {
                    return cadentero.ToUpper() + " y " + caddecimal + " " + obj.parm1maesgen;
                }
                return null;
            }
        }
